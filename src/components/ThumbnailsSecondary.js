...
import { SContainer, SThumbRow, SThumbBoxSec, SSideMenuContainer, SSideMenuItem, SThumbTitle, SThumbImageSec, SThumbTextSec, SThumbImageSide, SThumbTextSide} from "./styles/ThumnailsSecodary.styled";
    ...
    if (isLoading) {
        ...
        MainBox = posts.slice(0, 4).map((e, i) => (
            <Link to={`/page/${e.id}`} key= {i}>
                <SThumbBoxSec>
                    <CategoryIcon><IconSwitch target={category}/></CategoryIcon>
                    <SThumbImageSec src={`./images/articles/${e.id}.jpg`} alt={e.title} />
                    <SThumbTextSec>
                        <STitleS>{e.title}</STitleS>
                        <span><TimeAgo timestamp={e.date}/></span>
                    </SThumbTextSec> 
                </SThumbBoxSec>
            </Link>
            )
        ...
        SideBox = (
            <SSideMenuContainer>
                {posts.slice(4, 14).map((e, i) => (
                    <Link to={`/page/${e.id}`} key={i}>
                    <SSideMenuItem >
                            <SThumbImageSide src={`./images/articles/${e.id}.jpg`} alt={e.title} />
                            <SThumbTextSide>
                                <STitleS>{e.title}</STitleS>
                            </SThumbTextSide>
                    </SSideMenuItem>
